function blendSupportAdjustments() {
    ...
    (function ($) {
        ...
        if (videoAttachment.length) {
            videoAttachment.each(function () {
                const $this = $(this);
                const hrefSplice = $this.attr('href');
        ...
        const $body = $('body');
        const modal = $(modalEl);
        const modalTitle = modal.find('.js-modal__title');
        const modalMedia = modal.find('.js-modal__media');
        const modalCloseElements = $('.js-modal-backdrop, .js-btn__close');
        const modalNavButtons = modal.find('.js-modal__prev, .js-modal__next');
        ...
        if (imgAttachment.length) {
            const galleryHtml = $(`<div class="js-img-gallery"></div>`);
        ...
        $('ul.details > li').find(imgLink).not('[href*=".zip"]').each(function () {
            const $imgLink = $(this);
            const hrefSplice = $imgLink.attr('href');
        ...
        modalNavButtons.on('click', function (index) {
            ...
            const currentGroupElement = $('[data-group-index="' + $activeButton.attr('data-current-group-index') + '"]');
            const currentItemIndex = currentGroupElement.find('[data-nav-index="' + $activeButton.attr('data-current-img-index') +'"]').attr('data-nav-index');
            const groupItemCount = currentGroupElement.find('[data-nav-index]').length;
            let goToThisItem;
        ...
        modal.on('modalNavDone modalShowing', function () {
            const navButtons = modal.find('.js-modal__prev, .js-modal__next');
            const prevButton = modal.find('.js-modal__prev');
            const nextButton = modal.find('.js-modal__next');
